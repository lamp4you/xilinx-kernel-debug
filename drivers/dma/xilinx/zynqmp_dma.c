			   ...
			   struct platform_device *pdev)
{
    ...
    err = 0;    
    device_debug_enter(pdev_name, NULL, "zynqmp_dma_chan_probe : pdev(%s), pdev->dev(%s)\n", pdev_name, device_name);
	...
	if (IS_ERR(chan->regs)) {
		err = PTR_ERR(chan->regs); goto _done; // return PTR_ERR(chan->regs);
    }
    device_debug_msg(pdev_name, NULL, "res = %pR, chan->regs = 0x%016llx\n", res, ptr2uint64_t(chan->regs));
    ...
    device_debug_msg(pdev_name, NULL, "zynqmp_dma_init\n");
	zynqmp_dma_init(chan);
	chan->irq = platform_get_irq(pdev, 0);
	if (chan->irq < 0) {
		err = -ENXIO; goto _done; // return -ENXIO;
    }
...
_done:
    device_debug_exit(pdev_name, NULL, "zynqmp_dma_chan_probe : pdev(%s), pdev->dev(%s)\n", pdev_name, device_name);   
	return err; // return 0;
}
					    ...
					    struct of_dma *ofdma)
{
    ...
    eth_debug_msg("of_zynqmp_dma_xlate : chan(%s)\n", dma_chan_name(&zdev->chan->common));    
...
static int zynqmp_dma_probe(struct platform_device *pdev)
{
    ...
    const char *device_name = dev_name(&pdev->dev);
    ret = 0;
    device_debug_enter(pdev_name, NULL, "zynqmp_dma_probe : pdev(%s), pdev->dev(%s)\n", pdev_name, device_name);
	...
	if (IS_ERR(zdev->clk_main)) {
		...
		ret = PTR_ERR(zdev->clk_main); goto _done; // return PTR_ERR(zdev->clk_main);
	}

	zdev->clk_apb = devm_clk_get(&pdev->dev, "clk_apb");
	if (IS_ERR(zdev->clk_apb)) {
		dev_err(&pdev->dev, "apb clock not found.\n");
		ret = PTR_ERR(zdev->clk_apb); goto _done; // return PTR_ERR(zdev->clk_apb);
	}
...
_done:
    device_debug_exit(pdev_name, NULL, "zynqmp_dma_probe : pdev(%s), pdev->dev(%s)\n", pdev_name, device_name);    
	return ret; // return 0;
