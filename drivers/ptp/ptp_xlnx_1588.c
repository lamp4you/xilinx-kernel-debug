...
/* User needs to update XTIMER_CLK_PERIOD_PICSEC based on the h/w design timer clock
 * period value.
 */
#define XTIMER_CLK_PERIOD_PICSEC	2
#define XTIMER_FRACTION_OFONE_NSEC	1048576
...
static int xlnx_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	...
	if (sign)
		offset = timespec64_sub(offset, then);
	else
			  ...
			  struct ptp_clock_request *rq, int on)
{
    ...
    int ret = -EOPNOTSUPP;
      ptp_debug_enter("xlnx_ptp_enable\n");    
 ...
 * second. When our count reaches 128 emit a PTP_CLOCK_PPS event
 */
static irqreturn_t xlnx_ptp_timer_isr(int irq, void *priv)
{
    ...
    if ((timer->ptp_clock) && (timer->extts_enable)) {
        ...
        sec = in_be32(timer->baseaddr + XTIMER1588_1PPS_TSTAMP_SEC);
...
static int xlnx_ptp_timer_probe(struct platform_device *pdev)
{
    ...
    ptp_debug_enter("xlnx_ptp_timer_probe : pdev(%s)\n", pdev->name);    
    ...
    ptp_debug_msg("r_mem = %pR, timer->baseaddr = 0x016llx\n", r_mem, ptr2uint64_t(timer->baseaddr));
    ...
    ptp_debug_msg("ptp_clock_register : pdev(%s)\n", pdev->name);
	...
	timer->incval = XTIMER_CLK_PERIOD_PICSEC * XTIMER_FRACTION_OFONE_NSEC;

	err = xlnx_ptp_timer_request_irq(timer);
	if (err)
		goto out;

	out_be32((timer->baseaddr + XTIMER1588_RTC_INCREMENT), timer->incval);
...
out:
	timer->ptp_clock = NULL;
_done:
    ptp_debug_exit("xlnx_ptp_timer_probe : pdev(%s)\n", pdev->name);
	return err;
}
