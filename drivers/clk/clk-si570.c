...
static int si570_get_defaults(struct clk_si570 *data, u64 fout)
{
    ...
    dev_debug_enter("si570_get_defaults\n");
    ...
    dev_debug_msg("n1 = %u, hs_div = %u, rfreq = %lld, fdco = %llu, fxtal = %llu\n", data->n1, data->hs_div, data->rfreq, fdco, data->fxtal);
    dev_debug_msg("frequency = %llu Hz\n", data->frequency);

_done:
    dev_debug_exit("si570_get_defaults\n");
	return err; // return 0;
		...
		const struct i2c_device_id *id)
{
    ...
    const char *device_name = dev_name(&client->dev);
    dev_debug_enter("si570_probe : dev(%s)\n", device_name);
    ...
    dev_debug_msg("factory-fout : %ld Hz\n", factory_fout);

	data->regmap = devm_regmap_init_i2c(client, &si570_regmap_config);
	if (IS_ERR(data->regmap)) {
		dev_err(&client->dev, "failed to allocate register map\n");
		err =  PTR_ERR(data->regmap); goto _done; // return PTR_ERR(data->regmap);
	}
...
_done:
    dev_debug_exit("si570_probe : dev(%s)\n", device_name);
	return err; // return 0;
}
