...
static const char *get_net_device_name(struct net_device *dev)
{
    if(dev) {
        return (const char *)dev->name;
    }
    else {
...
static int macvlan_open(struct net_device *dev)
{
	...
	struct net_device *lowerdev = vlan->lowerdev;
	int err;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_open : ndev(%s)\n", ndev_name);
...
out:
    eth_debug_exit("macvlan_open : ndev(%s)\n", ndev_name);
	return err;
}
...
static int macvlan_sync_address(struct net_device *dev, unsigned char *addr)
{
	...
	int err;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_sync_address : ndev(%s)\n", ndev_name);
	if (!(dev->flags & IFF_UP)) {
		...
		if (macvlan_addr_busy(vlan->port, addr)) {
			err = -EBUSY; goto _done; // return -EBUSY;
        }
...
_done:    
    eth_debug_exit("macvlan_sync_address : ndev(%s)\n", ndev_name);
	return err; // return 0;
}
...
static int macvlan_set_mac_address(struct net_device *dev, void *p)
{
    ...
    int ret = 0;
	const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_set_mac_address : ndev(%s)\n", ndev_name);
	if (!is_valid_ether_addr(addr->sa_data)) {
		ret =  -EADDRNOTAVAIL; goto _done; // return -EADDRNOTAVAIL;
    }
	...
	if (vlan->mode == MACVLAN_MODE_PASSTHRU) {
		...
		ret = dev_set_mac_address(vlan->lowerdev, addr); goto _done; // return dev_set_mac_address(vlan->lowerdev, addr);
	}
	ret = macvlan_sync_address(dev, addr->sa_data); // return macvlan_sync_address(dev, addr->sa_data);

_done:
    eth_debug_exit("macvlan_set_mac_address : ndev(%s)\n", ndev_name);
    return ret;
}
...
static void macvlan_change_rx_flags(struct net_device *dev, int change)
{
	...
	struct net_device *lowerdev = vlan->lowerdev;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_change_rx_flags : ndev(%s)\n", ndev_name);
	if (dev->flags & IFF_UP) {
    ...
    eth_debug_exit("macvlan_change_rx_flags : ndev(%s)\n", ndev_name);
}
...
{
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_compute_filter : ndev(%s)\n", ndev_name);
    ...
    eth_debug_exit("macvlan_compute_filter : ndev(%s)\n", ndev_name);
}
...
static void macvlan_set_mac_lists(struct net_device *dev)
{
	struct macvlan_dev *vlan = netdev_priv(dev);
	const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_set_mac_lists : ndev(%s)\n", ndev_name);
    ...
    eth_debug_exit("macvlan_set_mac_lists : ndev(%s)\n", ndev_name);
}
...
static int macvlan_change_mtu(struct net_device *dev, int new_mtu)
{
    ...
    int ret = 0;
	const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_change_mtu : ndev(%s)\n", ndev_name);
	if (vlan->lowerdev->mtu < new_mtu) {
		ret = -EINVAL; goto _done; // return -EINVAL;
    }
	dev->mtu = new_mtu;
_done:
    eth_debug_exit("macvlan_change_mtu : ndev(%s)\n", ndev_name);
	return ret; // return 0;
}
...
static int macvlan_init(struct net_device *dev)
{
    ...
    int ret = 0;
	const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_init : ndev(%s)\n", ndev_name);
...
_done:
    eth_debug_exit("macvlan_init : ndev(%s)\n", ndev_name);
	return ret; // return 0;
}
				   ...
				   __be16 proto, u16 vid)
{
    ...
    int ret;
	const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_vlan_rx_add_vid : ndev(%s)\n", ndev_name);
	ret =  vlan_vid_add(lowerdev, proto, vid); // return vlan_vid_add(lowerdev, proto, vid);
    eth_debug_exit("macvlan_vlan_rx_add_vid : ndev(%s)\n", ndev_name);
    return ret;
}
			   ...
			   u16 flags)
{
	...
	int err = -EINVAL;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_fdb_add : ndev(%s)\n", ndev_name);
...
_done:
    eth_debug_exit("macvlan_fdb_add : ndev(%s)\n", ndev_name);
	return err;
}
			   ...
			   const unsigned char *addr, u16 vid)
{
	...
	int err = -EINVAL;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_fdb_del : ndev(%s)\n", ndev_name);
	/* Support unicast filter only on passthru devices.
	 * Multicast filter should be allowed on all devices.
	 */
	if (!macvlan_passthru(vlan->port) && is_unicast_ether_addr(addr)) {
...
_done:
    eth_debug_exit("macvlan_fdb_del : ndev(%s)\n", ndev_name);
	return err;
}
...
{
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_msg("macvlan_ethtool_get_drvinfo : ndev(%s)\n", ndev_name);
					      ...
					      struct ethtool_link_ksettings *cmd)
{
    ...
    int ret;
	const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_ethtool_get_link_ksettings : ndev(%s)\n", ndev_name);
	ret = __ethtool_get_link_ksettings(vlan->lowerdev, cmd); // return __ethtool_get_link_ksettings(vlan->lowerdev, cmd);
    eth_debug_exit("macvlan_ethtool_get_link_ksettings : ndev(%s)\n", ndev_name);
    return ret;
}
					      ...
					      netdev_features_t features)
{
    ...
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_fix_features : ndev(%s)\n",ndev_name);
    ...
    eth_debug_exit("macvlan_fix_features : ndev(%s)\n", ndev_name);
...
{
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_setup : ndev(%s)\n", ndev_name);
	macvlan_common_setup(dev);
	dev->priv_flags |= IFF_NO_QUEUE;
    eth_debug_exit("macvlan_setup : ndev(%s)\n", ndev_name);
}
...
static int macvlan_port_create(struct net_device *dev)
{
	...
	int err;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_port_create : ndev(%s)\n", ndev_name);
	...
	if (port == NULL) {
		err = -ENOMEM; goto _done; // return -ENOMEM;
    }
...
_done:
    eth_debug_exit("macvlan_port_create : ndev(%s)\n", ndev_name);    
	return err;
}
				      ...
				      struct nlattr *data[])
{
    ...
    ret = 0;
    eth_debug_enter("macvlan_changelink_sources\n");
	if (data[IFLA_MACVLAN_MACADDR])
		addr = nla_data(data[IFLA_MACVLAN_MACADDR]);

	if (mode == MACVLAN_MACADDR_ADD) {
        eth_debug_msg("MACVLAN_MACADDR_ADD\n");
		if (!addr) {
			ret = -EINVAL; goto _done; // return -EINVAL;
        }
		ret = macvlan_hash_add_source(vlan, addr); goto _done; // return macvlan_hash_add_source(vlan, addr);

	} else if (mode == MACVLAN_MACADDR_DEL) {
        eth_debug_msg("MACVLAN_MACADDR_DEL\n");
		if (!addr) {
			ret = -EINVAL; goto _done; // return -EINVAL;
        }
	    ...
	    eth_debug_msg("MACVLAN_MACADDR_FLUSH\n");
		macvlan_flush_sources(vlan->port, vlan);
	} else if (mode == MACVLAN_MACADDR_SET) {
        eth_debug_msg("MACVLAN_MACADDR_SET\n");
		macvlan_flush_sources(vlan->port, vlan);
...
_done:
    eth_debug_exit("macvlan_changelink_sources\n");
	return ret; // return 0;
}
			   ...
			   struct netlink_ext_ack *extack)
{
	...
	bool create = false;
    const char *ndev_name = get_net_device_name(dev);
    eth_debug_enter("macvlan_common_newlink : ndev(%s)\n", ndev_name);
	if (!tb[IFLA_LINK]) {
		err = -EINVAL; goto _done; // return -EINVAL;
    }
...
destroy_macvlan_port:
	...
	if (create && macvlan_port_get_rtnl(lowerdev))
		macvlan_port_destroy(port->dev);
_done:
    eth_debug_exit("macvlan_common_newlink : ndev(%s)\n", ndev_name);
	return err;
}
			      ...
			      struct netlink_ext_ack *extack)
{
    ...
    ret = 0;
    eth_debug_enter("macvlan_changelink : ndev(%s)\n", ndev_name);
...
_done:
    eth_debug_exit("macvlan_changelink: ndev(%s)\n", ndev_name);   
	return ret; // return 0;
}
...
{
	int err;
    eth_debug_enter("macvlan_init_module\n");
...
_done:    
    eth_debug_exit("macvlan_init_module\n");    
	return err;
}
