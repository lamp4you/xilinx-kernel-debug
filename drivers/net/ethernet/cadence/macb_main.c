...
static void macb_handle_link_change(struct net_device *dev)
{
    ...
    macb_debug_enter("macb_handle_link_change : ndev(%s)\n", dev->name);
	...
	if (status_change) {
		if (phydev->link) {
			...
			netdev_info(dev, "link up (%d/%s)\n",
				    ...
				    "Full" : "Half");
            macb_debug_msg("link up (%d/%s)\n", phydev->speed, phydev->duplex == DUPLEX_FULL ? "Full" : "Half");
		} else {
			netif_carrier_off(dev);
			netdev_info(dev, "link down\n");
            macb_debug_msg("link down\n");
		}
	}
    macb_debug_exit("macb_handle_link_change : ndev(%s)\n", dev->name);
}
...
static int macb_mii_probe(struct net_device *dev)
{
    ...
    macb_debug_enter("macb_mii_probe : net_device(%s)\n", dev->name);
	pdata = dev_get_platdata(&bp->pdev->dev);
    ...
    macb_debug_exit("macb_mii_probe : net_device(%s)\n", dev->name);
...
static int macb_open(struct net_device *dev)
{
    ...
    macb_debug_enter("macb_open : ndev(%s)\n", dev->name);
	...
	if (!dev->phydev) {
		err = -EAGAIN; goto _done; // return -EAGAIN;
    }
    ...
    err = 0;
_done:
    macb_debug_exit("macb_open : ndev(%s)\n", dev->name);    
	return err; // return 0;
}
			 ...
			 struct clk **rx_clk, struct clk **tsu_clk)
{
    ...
    err = 0;
    macb_debug_enter("macb_clk_init : pdev(%s)\n", pdev->name);
	*pclk = devm_clk_get(&pdev->dev, "pclk");
	if (IS_ERR_OR_NULL(*pclk)) {
        ...
        macb_debug_msg("failed to get macb_clk (%u)\n", err);
		goto _done; // return err;
	}
	...
	*hclk = devm_clk_get(&pdev->dev, "hclk");
	if (IS_ERR_OR_NULL(*hclk)) {
        ...
        macb_debug_msg("failed to get hclk (%u)\n", err);
		goto _done; // return err;
	}
	...
	*tsu_clk = devm_clk_get(&pdev->dev, "tsu_clk");
	...
	if (err) {
        ...
        macb_debug_msg("failed to enable pclk (%u)\n", err);
		goto _done; // return err;
	}
	...
	if (err) {
        ...
        macb_debug_msg("failed to enable hclk (%u)\n", err);
	...
	if (err) {
        ...
        macb_debug_msg("failed to enable tx_clk (%u)\n", err);
	...
	if (err) {
        ...
        macb_debug_msg("failed to enable rx_clk (%u)\n", err);
	...
	if (err) {
        ...
        macb_debug_msg("failed to enable tsu_clk (%u)\n", err);
...
_done:
    macb_debug_exit("macb_clk_init : pdev(%s)\n", pdev->name);
	return err;
}
...
static int macb_init(struct platform_device *pdev)
{
	...
	int err;
	u32 val, reg;
    macb_debug_enter("macb_init : pdev(%s)\n", pdev->name);
	...
	if (macb_is_gem(bp)) {
        macb_debug_msg("setup appropriated routines according to adapter type : GEM\n");
        ...
        macb_debug_msg("setup appropriated routines according to adapter type : MACB\n");
    ...
    err = 0;
_done:
    macb_debug_exit("macb_init : pdev(%s)\n", pdev->name);
	return err; // return 0;
}
...
static int macb_probe(struct platform_device *pdev)
{
    ...
    const char *device_name = get_platform_device_name(pdev);
    const char *node_name = get_device_node_name(np);
    err = 0;
    macb_debug_enter("macb_probe : pdev(%s), node(%s)\n", device_name, node_name);
	regs = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	mem = devm_ioremap_resource(&pdev->dev, regs);
	if (IS_ERR(mem)) {
		err = (int)PTR_ERR(mem); goto _done; // return PTR_ERR(mem);
    }
    macb_debug_msg("regs = %pR, mem = 0x%016llx\n", regs, mem);
    ...
    macb_debug_msg("native_io = %d\n", native_io);

	macb_probe_queues(mem, native_io, &queue_mask, &num_queues);
    macb_debug_msg("alloc_etherdev_mq - num_queues(%d)\n", num_queues);
    ...
    if(regs->name) {
        macb_debug_msg("regs->name = %s\n", regs->name);
    }
    macb_debug_msg("regs->start = 0x%018lx\n", (unsigned long)regs->start);
    macb_debug_msg("regs->end = 0x%018lx\n", (unsigned long)regs->end);
	...
	if (macb_config) {
        ...
        macb_debug_msg("bp->dma_burst_length = %d\n", bp->dma_burst_length);
    }
	bp->pclk = pclk;
	bp->hclk = hclk;
	bp->tx_clk = tx_clk;
	...
	if (tsu_clk) {
        ...
        macb_debug_msg("bp->tsu_rate = %ld\n", (unsigned long)bp->tsu_rate);
    }

	if (macb_config) {
		bp->jumbo_max_len = macb_config->jumbo_max_len;
        macb_debug_msg("bp->jumbo_max_len = %ld\n", (unsigned long)bp->jumbo_max_len);
    }
    ...
    macb_debug_msg("set mac address\n");
    ...
    macb_debug_msg("Power up the PHY if there is a GPIO reset\n");
    ...
    macb_debug_msg("of_get_phy_mode\n");
    ...
    macb_debug_msg("bp->phy_interface = %s (%d)\n", phy_modes(bp->phy_interface), bp->phy_interface);

	/* IP specific init */
    macb_debug_msg("init dev(%s)\n", device_name);
	err = init(pdev);
	if (err)
		goto err_out_phy_put;

    macb_debug_msg("register_netdev\n");
    ...
    macb_debug_msg("netif_carrier_off\n");
	netif_carrier_off(dev);

    macb_debug_msg("tasklet_init\n");
    ...
    macb_debug_msg("phy_attached_info\n");
