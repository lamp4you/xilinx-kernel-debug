...
#define XXVENET_TS_HEADER_NO_TS 0 /* Dummy timestamp value used to disable further timestamping */
...
struct value_name axienet_ip_type_info_tbl[] =
{
    { XAXIENET_1G, "XAXIENET_1G" },
    { XAXIENET_2_5G, "XAXIENET_2_5G" },
    { XAXIENET_LEGACY_10G, "XAXIENET_LEGACY_10G" },
    { XAXIENET_10G_25G, "XAXIENET_10G_25G" },
    { -1, NULL }
};

const char *axienet_ip_type_name(int mactype)
{
    int i;

    for(i = 0; axienet_ip_type_info_tbl[i].name != NULL; i++) {
        if(axienet_ip_type_info_tbl[i].value == mactype) {
            return axienet_ip_type_info_tbl[i].name;
        }
    }
    return (const char *)"Unknown MACType";
}
...
void axienet_set_mac_address(struct net_device *ndev, const void *address)
{
    ...
    axi_net_debug_enter("axienet_set_mac_address : ndev(%s)\n", ndev->name);
...
_done:
    axi_net_debug_exit("axienet_set_mac_address : ndev(%s)\n", ndev->name);
...
static int netdev_set_mac_address(struct net_device *ndev, void *p)
{
    ...
    axi_net_debug_enter("netdev_set_mac_address : ndev(%s)\n", ndev->name);

	axienet_set_mac_address(ndev, addr->sa_data);
    
    axi_net_debug_exit("netdev_set_mac_address : ndev(%s)\n", ndev->name);
...
void __axienet_device_reset(struct axienet_dma_q *q, off_t offset)
{
	 ...
	 * reset process.
	 */
    axi_net_debug_msg("__axienet_device_reset : q->dma_reg = 0x%016llx, offset = %08lx\n", ptr2uint64_t(q->dma_regs), offset);
	axienet_dma_out32(q, offset, XAXIDMA_CR_RESET_MASK);
	timeout = DELAY_OF_ONE_MILLISEC;
#if 1
    while(1) {
        u32 val = axienet_dma_in32(q, offset);
        if((timeout % 200) == 0) {
            axi_net_debug_msg("q->dma_reg = 0x%016llx, offset = %08lx, value = 0x%08lx\n", ptr2uint64_t(q->dma_regs), offset, val);
        }
        if((val & XAXIDMA_CR_RESET_MASK) == 0) {
            break;
        }
        udelay(1);
        if (--timeout == 0) {
...
struct value_name axienet_regs[] =
{
    { XXV_GT_RESET_OFFSET,      "GT_RESET" },
    { XXV_RESET_REG_OFFSET,     "RESET_REG" },
    { XXV_MODE_REG_OFFSET,      "MODE_REG" },    
    { XXV_TC_OFFSET,            "CONFIGURATION_TX_REG1" },    
    { XXV_RCW1_OFFSET,          "CONFIGURATION_RX_REG1" },    
    { XXV_JUM_OFFSET,           "CONFIGURATION_RX_MTU" },    
    { XXV_VL_LENGTH_OFFSET,     "CONFIGURATION_VL_LENGTH_REG" },    
    { XXV_REVISION_REG_OFFSET,  "CONFIGURATION_REVISION_REG" },    
    // { XXV_STATRX_BLKLCK_OFFSET, "STATRX_BLKLCK" },    
    
    { -1, NULL } 
};

static void axienet_device_dump_reg(struct axienet_local *lp)
{
    u32 val;
    int i;

    for(i = 0; axienet_regs[i].name != NULL; i++) {
        val = axienet_ior(lp, (off_t)axienet_regs[i].value);
        axi_net_debug_msg("read 0x%08lx (%s), val 0x%08lx\n", axienet_regs[i].value, axienet_regs[i].name, val);
    }
}

#define DEBUG_READ_MAX_COUNT    5

int debug_read_l_cnt = 0;
u32 axienet_readl(const volatile void __iomem *addr)
{
    u32 __v;

    if(debug_read_l_cnt > 0) {
        axi_net_debug_msg("axienet_readl (0x%016llx)\n", ptr2uint64_t(addr));
    }

    __v = le32_to_cpu((__force __le32)__raw_readl(addr)); 
     __iormb();          

    if(debug_read_l_cnt > 0) {
        debug_read_l_cnt--;
        axi_net_debug_msg("axienet_readl (0x%016llx), value = 0x%08lx\n", ptr2uint64_t(addr), __v);
    }    

    return __v;
}

EXPORT_SYMBOL(axienet_readl);

static int axienet_device_read_poll_timeout(struct axienet_local *lp, off_t offset, u32 mask, u32 exit_cond, unsigned long sleep_us)
{
	u64 __timeout_us = DELAY_OF_ONE_MILLISEC;
	unsigned long __sleep_us = sleep_us; 
	ktime_t __timeout = ktime_add_us(ktime_get(), __timeout_us);
    u32 val;

    axi_net_debug_enter("axienet_device_read_poll_timeout : lp->regs = 0x%016llx, offset = 0x%08llx\n", ptr2uint64_t(lp->regs), offset);
    debug_read_l_cnt = DEBUG_READ_MAX_COUNT;
	might_sleep_if((__sleep_us) != 0);
	for (;;) {

        val = axienet_readl(lp->regs + offset);
		if ((val & mask) == exit_cond) {
			break;
        }
    ...
    axi_net_debug_exit("axienet_device_read_poll_timeout : lp->regs = 0x%016llx, offset = 0x%08llx\n", ptr2uint64_t(lp->regs), offset);    
    return ((val & mask) == exit_cond) ? 0 : -ETIMEDOUT;    
}
...
static void axienet_device_reset(struct net_device *ndev)
{
    ...
    axi_net_debug_enter("axienet_device_reset : net_dev(%s), dev(%s)\n", ndev->name, dev_name(lp->dev));

	if (lp->axienet_config->mactype == XAXIENET_10G_25G) {
        axi_net_debug_msg("Reset the XXV MAC\n");    
		/* Reset the XXV MAC */
		val = axienet_ior(lp, XXV_GT_RESET_OFFSET);
        axi_net_debug_msg("read 0x%08lx (GT_RESET), val 0x%08lx\n", XXV_GT_RESET_OFFSET, val);
    ...
    axi_net_debug_msg("Dump Registers\n");
    axienet_device_dump_reg(lp);
	...
	if (!lp->is_tsn || lp->temac_no == XAE_TEMAC1) {
        axi_net_debug_msg("axienet_dma_bd_init\n"); 
	...
	if (lp->axienet_config->mactype != XAXIENET_10G_25G) {
        axi_net_debug_msg("XAE_RCW1_OFFSET\n"); 
	...
	if (lp->axienet_config->mactype == XAXIENET_10G_25G) {
        axi_net_debug_msg("readl_poll_timeout\n"); 
...
#if 1
        err = axienet_device_read_poll_timeout(lp, XXV_STATRX_BLKLCK_OFFSET, XXV_RX_BLKLCK_MASK, XXV_RX_BLKLCK_MASK, 10);
#else        
		...
		if (!lp->is_tsn) {
            axi_net_debug_msg("reset Rx Timestamp - Rx Fifo reset\n"); 
			axienet_rxts_iow(lp, XAXIFIFO_TXTS_RDFR,
					 XAXIFIFO_TXTS_RESET_MASK);
        
            axi_net_debug_msg("reset Rx Timestamp - AXI4-Stream Reset\n"); 
			axienet_rxts_iow(lp, XAXIFIFO_TXTS_SRR,
					 XAXIFIFO_TXTS_RESET_MASK);
		}
#endif
        axi_net_debug_msg("readl_poll_timeout : Done\n"); 
	}
	...
	if ((lp->axienet_config->mactype == XAXIENET_1G) &&
        ...
        axi_net_debug_msg("XAE_IP_OFFSET\n"); 
    ...
    axi_net_debug_exit("axienet_device_reset : net_dev(%s), dev(%s)\n", ndev->name, dev_name(lp->dev));
...
#endif
{
	...
	if (val != cur_p->ptp_tx_ts_tag) {
		...
		dev_info(lp->dev, "2-step tag slip. Correcting...\n");
		while (count) {
			nsec = axienet_txts_ior(lp, XAXIFIFO_TXTS_RXFD);
			sec  = axienet_txts_ior(lp, XAXIFIFO_TXTS_RXFD);
			val = axienet_txts_ior(lp, XAXIFIFO_TXTS_RXFD);
		...
		if (val != cur_p->ptp_tx_ts_tag) {
			dev_info(lp->dev, "Mismatching 2-step tag.\n");
			dev_info(lp->dev, "Got %x, Expected %x\n",
		...
		if ((((lp->tstamp_config.tx_type == HWTSTAMP_TX_ONESTEP_SYNC) ||
            ...
            if (skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP) {
                cur_p->ptp_tx_ts_tag = prandom_u32() &
                                        ~XAXIFIFO_TXTS_TAG_MASK;
                /* Avoid 0 as a tag. */
                if (cur_p->ptp_tx_ts_tag == 0)
                    cur_p->ptp_tx_ts_tag++;
            ...
            } else { /* No timestamp needed for this packet */
                axienet_txts_iow(lp, XAXIFIFO_TXTS_TXFD,
                        XXVENET_TS_HEADER_NO_TS);
...
static int axienet_open(struct net_device *ndev)
{
    ...
    axi_net_debug_enter("axienet_open : net_dev(%s), dev(%s)\n", ndev->name, dev_name(lp->dev));
	dev_dbg(&ndev->dev, "axienet_open()\n");

    axi_net_debug_msg("mactype : %s (%d)\n", axienet_ip_type_name(lp->axienet_config->mactype), lp->axienet_config->mactype);
	if (lp->axienet_config->mactype == XAXIENET_10G_25G)
		axienet_device_reset(ndev);
	else
		ret = axienet_mii_init(ndev);
	if (ret < 0) {
		goto _done; // return ret;
    }
	...
	if (lp->phy_mode == XXE_PHY_TYPE_USXGMII) {
		...
		if (err) {
			netdev_err(ndev, "%s: USXGMII Block lock bit not set",
				   __func__);
			ret = -ENODEV; goto _done; // return -ENODEV;
		}
...
_done:
    axi_net_debug_exit("axienet_open : net_dev(%s), dev(%s)\n", ndev->name, dev_name(lp->dev)); 
	return ret;
}
...
static int axienet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    int ret = 0;
    axi_net_debug_enter("axienet_ioctl : ndev(%s)\n", dev->name);
	if (!netif_running(dev)) {
		ret = -EINVAL; goto _done; // return -EINVAL;
    }
	...
	case SIOCSMIIREG:
		if (!dev->phydev) {
			ret = -EOPNOTSUPP; goto _done; // return -EOPNOTSUPP;
        }
		ret = phy_mii_ioctl(dev->phydev, rq, cmd); goto _done; // return phy_mii_ioctl(dev->phydev, rq, cmd);
		
#if defined (CONFIG_XILINX_AXI_EMAC_HWTSTAMP) || defined (CONFIG_XILINX_TSN_PTP)
	case SIOCSHWTSTAMP:
		ret = axienet_set_ts_config(lp, rq); goto _done; // return axienet_set_ts_config(lp, rq);
	case SIOCGHWTSTAMP:
		ret = axienet_get_ts_config(lp, rq); goto _done; // return axienet_get_ts_config(lp, rq);
#endif
#ifdef CONFIG_XILINX_TSN_QBV
	case SIOCCHIOCTL:
		ret = axienet_set_schedule(dev, rq->ifr_data); goto _done; // return axienet_set_schedule(dev, rq->ifr_data);
	case SIOC_GET_SCHED:
		ret = axienet_get_schedule(dev, rq->ifr_data); goto _done; // return axienet_get_schedule(dev, rq->ifr_data);
#endif
#ifdef CONFIG_XILINX_TSN_QBR
	case SIOC_PREEMPTION_CFG:
		ret = axienet_preemption(dev, rq->ifr_data); goto _done; // return axienet_preemption(dev, rq->ifr_data);
	case SIOC_PREEMPTION_CTRL:
		ret = axienet_preemption_ctrl(dev, rq->ifr_data); goto _done; // return axienet_preemption_ctrl(dev, rq->ifr_data);
	case SIOC_PREEMPTION_STS:
		ret = axienet_preemption_sts(dev, rq->ifr_data); goto _done; // return axienet_preemption_sts(dev, rq->ifr_data);
	case SIOC_PREEMPTION_COUNTER:
		ret = axienet_preemption_cnt(dev, rq->ifr_data); goto _done; // return axienet_preemption_cnt(dev, rq->ifr_data);
#ifdef CONFIG_XILINX_TSN_QBV
	case SIOC_QBU_USER_OVERRIDE:
		ret = axienet_qbu_user_override(dev, rq->ifr_data); goto _done; // return axienet_qbu_user_override(dev, rq->ifr_data);
	case SIOC_QBU_STS:
		ret = axienet_qbu_sts(dev, rq->ifr_data); goto _done; // return axienet_qbu_sts(dev, rq->ifr_data);
#endif
#endif
	default:
		ret = -EOPNOTSUPP; goto _done; // return -EOPNOTSUPP;
...
_done:
    axi_net_debug_exit("axienet_ioctl : ndev(%s)\n", dev->name);
    return ret;
					    ...
					    struct net_device *ndev)
{
    ...
    ret = 0;
    axi_net_debug_enter("axienet_dma_probe : pdev(%s), ndev(%s)\n", get_platform_device_name(pdev), ndev->name);
	...
	for_each_rx_dma_queue(lp, i) {
        ...
        axi_net_debug_msg("axistream-connected - node(%s)\n", get_device_node_name(np));
		if (np) {
			ret = of_address_to_resource(np, 0, &dmares);
			if (ret >= 0) {
				q->dma_regs = devm_ioremap_resource(&pdev->dev,
								&dmares);
                axi_net_debug_msg("dmares = %pR, q->dma_regs = 0x%016llx\n", dmares, ptr2uint64_t(q->dma_regs));
            }
			else {
				ret = -ENODEV; goto _done; // return -ENODEV;
            }
...
_done:
    axi_net_debug_exit("axienet_dma_probe : pdev(%s), ndev(%s)\n", get_platform_device_name(pdev), ndev->name);
	return ret; // return 0;
...
static int axienet_dma_clk_init(struct platform_device *pdev)
{
    ...
    axi_net_debug_enter("axienet_dma_clk_init : pdev(%s)\n", get_platform_device_name(pdev));
	...
	if (IS_ERR(lp->dma_tx_clk)) {
        ...
        axi_net_debug_msg("devm_clk_get - m_axi_mm2s_aclk (0x%016llx)\n", ptr2uint64_t(lp->dma_tx_clk));
	} else {
    ...
    axi_net_debug_msg("devm_clk_get - m_axi_s2mm_aclk (0x%016llx)\n", ptr2uint64_t(lp->dma_rx_clk));
    ...
    axi_net_debug_msg("devm_clk_get - m_axi_sg_aclk (0x%016llx)\n", ptr2uint64_t(lp->dma_sg_clk));


    axi_net_debug_msg("clk_prepare_enable - lp->dma_tx_clk\n");
    ...
    axi_net_debug_msg("clk_prepare_enable - lp->dma_rx_clk\n");
    ...
    axi_net_debug_msg("clk_prepare_enable - lp->dma_sg_clk\n");
...
_done:
    axi_net_debug_exit("axienet_dma_clk_init : pdev(%s)\n", get_platform_device_name(pdev));
	return err;
}
...
static int axienet_probe(struct platform_device *pdev)
{
    ...
    axi_net_debug_enter("axienet_probe : pdev(%s)\n", get_platform_device_name(pdev));
    ...
    axi_net_debug_msg("lp = 0x%016llx\n", ptr2uint64_t(lp));
#ifdef CONFIG_XILINX_TSN
    ...
    axi_net_debug_msg("pdev = 0x%016llx, pdev->num_resources = %d\n", ptr2uint64_t(pdev), pdev->num_resources);
	ethres = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	lp->regs = devm_ioremap_resource(&pdev->dev, ethres);
    axi_net_debug_msg("ethres = %pR, lp->regs = 0x%016llx\n", ethres, ptr2uint64_t(lp->regs));
	...
	if (!lp->is_tsn) {
        ...
        axi_net_debug_msg("node(%s), txtsres = %pR, lp->tx_ts_regs = 0x%016llx\n", get_device_node_name(np), txtsres, ptr2uint64_t(lp->tx_ts_regs));
		...
		if (lp->axienet_config->mactype == XAXIENET_10G_25G) {
            ...
            axi_net_debug_msg("node(%s), rxtsres = %pR, lp->rx_ts_regs = 0x%016llx\n", get_device_node_name(np), rxtsres, ptr2uint64_t(lp->rx_ts_regs));

			lp->tx_ptpheader = devm_kzalloc(&pdev->dev,
						XXVENET_TS_HEADER_LEN,
						GFP_KERNEL);

            axi_net_debug_msg("lp->tx_ptpheader = 0x%016llx\n", ptr2uint64_t(lp->tx_ptpheader));
	...
	if (!slave) {
#ifdef CONFIG_AXIENET_HAS_MCDMA
        axi_net_debug_msg("axienet_mcdma_probe\n");
		ret = axienet_mcdma_probe(pdev, lp, ndev);
#else
        axi_net_debug_msg("axienet_dma_probe\n");
        ...
        axi_net_debug_msg("axienet_dma_clk_init\n");
    ...
    axi_net_debug_msg("axienet_clk_init\n");
    ...
    axi_net_debug_msg("of_get_mac_address\n");
    ...
    axi_net_debug_msg("of_get_phy_mode\n");
	ret = of_get_phy_mode(pdev->dev.of_node);
	if (ret < 0) {
		// dev_warn(&pdev->dev, "couldn't find phy i/f\n");
		axi_net_debug_msg("couldn't find phy i/f\n");
    }
	...
	if (lp->phy_node) {
        ...
        axi_net_debug_msg("No phy-node is in device tree.\n");
    }

#ifdef CONFIG_AXIENET_HAS_MCDMA
	/* Create sysfs file entries for the device */
    axi_net_debug_msg("axeinet_mcdma_create_sysfs\n");
	ret = axeinet_mcdma_create_sysfs(&lp->dev->kobj);
	if (ret < 0) {
		dev_err(lp->dev, "unable to create sysfs entries\n");
		ret = -ENOMEM; goto _done; // return ret;
	}
#endif

    axi_net_debug_msg("register_netdev : lp->ndev(%s)\n", lp->ndev->name);
    ...
    axi_net_debug_msg("register_netdev : lp->ndev(%s) : DONE\n", lp->ndev->name);
...
_done:
    axi_net_debug_exit("axienet_probe : pdev(%s)\n", get_platform_device_name(pdev));
	return ret;
}
